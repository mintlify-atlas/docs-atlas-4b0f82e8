---
title: Chapter 1.3 - Functions & Addresses
description: Understanding how to hook functions using addresses and the $modify abstraction
---
...
<Info>
Manual hooks are sometimes necessary, for example to hook some obscure low-level functions that only exists on one platform, like GLFW on Windows. However, for 99% of mods, you should just be using `$modify`, since it makes your code much more portable and easier to work with.
</Info>
...
<Warning>
The above code snippet also references **calling conventions** - if you use `$modify`, Geode handles them for you, however if you're doing manual hooks or reverse engineering, these are very important to get right.
</Warning>
...
<Info>
Traditionally, the most common GD header library was [**gd.h**](https://github.com/HJfod/gd.h). However, nowadays **gd.h is completely obsolete**, as it is only for 2.1 and fully unmaintained.
</Info>
