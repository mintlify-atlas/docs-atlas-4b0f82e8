---
title: Modifying Geode UI
description: Customize Geode's own user interface with events
---
...
5. The mod must **give all of its own nodes IDs prefixed by its mod ID and check beforehand if its nodes have been added on all events**
...
<Note>
Nodes nested inside other nodes don't need to be prefixed, as long as the topmost parent is prefixed. For example, `my-mod.id/container > button` is completely fair!
</Note>
