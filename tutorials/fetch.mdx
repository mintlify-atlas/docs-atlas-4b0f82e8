...
description: Learn how to make asynchronous HTTP requests using Geode's WebRequest API
---
...
<Warning>
Once the `TaskHolder` goes out of scope, the request is cancelled, and the callback will never be called. You must store the holder, or use `async::spawn()` directly to globally listen to a request.
</Warning>
